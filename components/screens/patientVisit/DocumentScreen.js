  ...
  const pickLabImage = async () => {
          ...
          if (!result.canceled) {
           ...
           // let filename_labImage = result.assets[0].uri.replace(/^.*[\\\/]/,"");
            let filename_labImage = result.assets[0].uri.split('/').pop();
...
const submitDocuments = () =>{
   ...
   let labresult_match = /\.(\w+)$/.exec(labresultFileName);
   let labresult_type = labresult_match ? `image/${labresult_match[1]}` : `image`;

   let labimage_match = /\.(\w+)$/.exec(labImageFileName);
   let labimage_type = labimage_match ? `image/${labimage_match[1]}` : `image`;

   let other_image_match = /\.(\w+)$/.exec(OtherImageFileName);
   let other_image_type = other_image_match ? `image/${other_image_match[1]}` : `image`;


  // formData.append('patient_picture', { uri: labresult, name: labresultFileName, type }); 


   AsyncStorage.setItem("labresult_type", labresult_type);
   AsyncStorage.setItem("labresult", labresult);
   AsyncStorage.setItem("labresultFileName", labresultFileName);

   AsyncStorage.setItem("labimage_type", labimage_type);
   AsyncStorage.setItem("labimage", labimage);
   AsyncStorage.setItem("labImageFileName", labImageFileName);

   AsyncStorage.setItem("other_image_type", other_image_type);
   AsyncStorage.setItem("other_image", other_image);
   AsyncStorage.setItem("OtherImageFileName", OtherImageFileName);
